...
class Category(SQLModel, table=True):
    ...
    name: str = Field(sa_column=Column(String(100, collation='Vietnamese_CI_AS')))
    description: Optional[str] = Field(default=None, sa_column=Column(String(500, collation='Vietnamese_CI_AS')))
...
class User(SQLModel, table=True):
    ...
    full_name: Optional[str] = Field(default=None, sa_column=Column(String(200, collation='Vietnamese_CI_AS')))
...
class Bank(SQLModel, table=True):
    ...
    id: int = Field(default=None, primary_key=True)
    name: str = Field(sa_column=Column(String(300, collation='Vietnamese_CI_AS')))
    short_name: str = Field(max_length=50)

class PaymentMethod(SQLModel, table=True):
    __tablename__ = "payment_method"  # Tên bảng tùy chỉnh
    id: int = Field(default=None, primary_key=True)
    name: str = Field(sa_column=Column(String(150, collation='Vietnamese_CI_AS')))
    description: Optional[str] = Field(default=None, sa_column=Column(String(500, collation='Vietnamese_CI_AS')))
...
class Event(SQLModel, table=True):
    ...
    id: int = Field(default=None, primary_key=True)
    name: str = Field(sa_column=Column(String(400, collation='Vietnamese_CI_AS')))
    description: Optional[str] = Field(default=None, sa_column=Column(Text(collation='Vietnamese_CI_AS')))
    ...
    location: str = Field(sa_column=Column(String(500, collation='Vietnamese_CI_AS')))
    poster: Optional[str] = Field(default=None, max_length=500)  # Event poster URL
    ...
    censored_at: Optional[str] = Field(default=None)
...
class TicketClass(SQLModel, table=True):
    ...
    name: str = Field(sa_column=Column(String(300, collation='Vietnamese_CI_AS')))
    description: Optional[str] = Field(default=None, sa_column=Column(String(800, collation='Vietnamese_CI_AS')))
...
class Ticket(SQLModel, table=True):
    ...
    user_id: int = Field(default=None, foreign_key="users.id")
    seat_id: Optional[int] = Field(default=None, foreign_key="event_seats.id")
...
class JWT(SQLModel, table=True):
    ...
    created_at: str = Field(default_factory=lambda: datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
    expires_at: str = Field(default_factory=lambda: (datetime.now() + timedelta(minutes=int(os.getenv("ACCESS_TOKEN_EXPIRE_MINUTES", "60")))).strftime("%Y-%m-%d %H:%M:%S"))
